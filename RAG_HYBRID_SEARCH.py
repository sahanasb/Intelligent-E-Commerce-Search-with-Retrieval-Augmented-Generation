### RAG With Hybrid Retriever and BGE Reranker
import os
import asyncio
...
# Hybrid retriever: base retrieval → BGE rerank

class HybridRetriever(BaseRetriever):
    bm25: Any
    vector: Any
    reranker: Any
    weights: list = [0.5, 0.5]   # tune: more weight to BM25 for exact names

    @classmethod
    def from_documents(cls, docs: list[Document], vector_store, reranker):
        bm25 = BM25Retriever.from_documents(docs)
        bm25.k = 5
        vector = vector_store.as_retriever(search_kwargs={"k": 5})
        return cls(bm25=bm25, vector=vector, reranker=reranker)

    def _get_relevant_documents(self, query: str, *, run_manager=None):
        # EnsembleRetriever handles RRF (Reciprocal Rank Fusion) merging
        ensemble = EnsembleRetriever(
            retrievers=[self.bm25, self.vector],
            weights=self.weights
        )
        merged_docs = ensemble.invoke(query)
        # Optional: rerank the merged results
        return self.reranker.rank(query, merged_docs, top_n=3)
...
def _build_chain():
    ...
    all_docs = store.similarity_search("", k=1000)  # load full catalog
